...
> This file is the port of the upstream `docs/user/guide/providers.md` (+ `providers.zh.md`). Model configuration operates on the **runtime** (`llm-pi-ai` / `llm-deepseek` / `settings.yaml`) and is client-language independent — the Java SDK, Python SDK, and Web UI share the same configuration semantics.
...
The generated [plugin configuration catalog](../../docs/config-catalog.md) lists every supported field and default. The `dsh-llm-pi-ai` and `dsh-llm-deepseek` references own direct `settings.yaml` configuration, catalog resolution, reasoning controls, credentials, and adapter errors (upstream `packages/llm/`, language-neutral).
