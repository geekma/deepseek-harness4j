...
[中文](adding-a-tool.md) | English

> This file is the port of the upstream `docs/cookbook/adding-a-tool.md` (+ `.zh.md`). A model-facing tool is a **runtime-side TypeScript plugin** (`ctx.tools.register(defineTool(...))`), client-language independent. The Java/Python SDK only issues `session/prompt` and consumes session events; the tool contracts, UI render intents, and verification requirements hold for any client language.

Reference for the contracts a model-facing tool must satisfy. For an ordered first tool, follow upstream [Build a tool](../user/develop/basic/tool.md). `packages/shell/tool-bash` is the production-grade three-package example.
...
The neutral vocabulary lives in `dsh-tools`; tools never import a UI or transport type. Host/client runtimes map each `card` into their own view. The design and the why are in the render-intent-union Agent Note; `dsh-tool-fs` (generic/diff) and `dsh-tool-bash` (terminal) are the reference implementations.

## Verification

Follow the repository testing policy and the owning package's test documentation. A shipped model- or UI-visible change requires the assembled coverage specified there.
