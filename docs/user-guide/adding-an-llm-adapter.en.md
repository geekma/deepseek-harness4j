...
Keep wire types, request serialization, transport parsing, chunk translation, and the adapter class as separate responsibilities; `llm-deepseek` is the reference layout.

## Verification

Follow the repository testing policy (upstream `docs/testing.md`), which owns adapter coverage, real-provider checks, and published-entry requirements.
